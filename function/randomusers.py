...
def get_full_names(data: dict) -> list[str]:#1
    ...
    """
    full_names = list(map(lambda user: f"{user['name']['first']} {user['name']['last']}", data['results']))
...
def get_users_by_country(data: dict, country: str) -> list[dict]:#2
    ...
    """
    filter_country = filter(
        lambda user : user['location']['country'].lower() == country.lower(),data['results']
        )

    full_names = list(map(lambda user :f"name : {user['name']['first']} {user['name']['last']}" ,filter_country))
...
def count_users_by_gender(data: dict) -> dict:#3
    ...
    """
    males = list(filter(
        lambda user:user['gender'].lower() == 'male'.lower(),data['results']
    ))
    females = list(filter(
        lambda user:user['gender'].lower() == 'female'.lower(),data['results']
    ))
...
def get_emails_of_older_than(data: dict, age: int) -> list[str]:#4
    ...
    """
    age_filter = list(filter(lambda user: user['dob']['age'] > age ,data['results'] ))
    
    emails = list(map(lambda user : user['email'],age_filter))
...
def sort_users_by_age(data: dict, descending: bool = False) -> list[dict]:#5
    ...
    """
    age_sorted = list(sorted(
        data['results'],
        key = lambda user: user['dob']['age'],
        reverse=descending
    ))
    
    get_age_name = list(map(
...
def get_usernames_starting_with(data: dict, letter: str) -> list[str]:#6
    ...
    """
    startswith_letter_filter = list(filter(lambda user : user['login']['username'].lower().startswith(letter.lower()),data['results']))
    
    return startswith_letter_filter
...
def get_average_age(data: dict) -> float:#7
    ...
    Returns:
        float: Average age.
    """
    get_age = list(map(lambda user : user['dob']['age'],data['results']))

    return sum(get_age)/len(get_age)
...
def get_all_coordinates(data: dict) -> list[tuple[str, str]]:#9
    ...
    """
    get_cordinates_latitude = list(map(
        lambda user:(
    ...
    return get_cordinates_latitude
...
def find_users_in_timezone(data: dict, offset: str) -> list[dict]:#11
    ...
    full_names = list(map(lambda user :f"name : {user['name']['first']} {user['name']['last']} , city : {user['location']['city']}" ,timezona))
...
def get_registered_before_year(data: dict, year: int) -> list[dict]:#12
    ...
    """
    yers_ago_list = filter(
        lambda user: user['registered']['date'][:10] < year,
        data['results']
        )
    
    user = list(map(
       lambda user: f" name : {user['name']['first']} {user['name']['last']},registered : {user['registered']['age']}",
        yers_ago_list
    ))
...
def run_functions() -> None:
    ...
    # print(sort_users_by_age(randomuser_data,False))
    ...
    print(get_registered_before_year(randomuser_data,"2020"))
